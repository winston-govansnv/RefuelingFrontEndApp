...
export class ManageClientComponent implements OnInit {
  ...
  drivers: CustomerModel[];
  breadcrumpPath:any;
  private API_URL= environment.apiUrl;

  displayedColumns: string[] = ['id', 'clientName', 'custId', 'custSoldTo', 'custShipTo', 'shipTo','delete','edit'];
  dataSource;
  ...
  getClients(){
    this.http.get<CustomerModel[]>(this.API_URL + 'api/customer').subscribe(result => {
      console.log(result)
        this.drivers= result;  
        this.dataSource = new MatTableDataSource<CustomerModel>( this.drivers);  
...
removeDriver(id){
  this.http.delete(this.API_URL + 'api/customer?id='+ id).subscribe(result => {      
    this.getClients(); 
