...
export class ManageDriverComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'firstname', 'lastname', 'badgeNumber', 'departmentName','delete','edit'];
  dataSource;
  ...
  openModalDriver(element) {
    const dialogRef = this.matDialog.open(DriverModalComponent, {
      height:"340px",
      width: '550px',
  ...
  getDrivers(){
    this.http.get<DriverModel[]>(this.API_URL + 'api/driver').subscribe(result => {
        this.drivers= result;  
        this.dataSource = new MatTableDataSource<DriverModel>( this.drivers);  
...
removeDriver(id){
  this.http.delete(this.API_URL + 'api/driver?id='+ id).subscribe(result => {      
    this.getDrivers();  
